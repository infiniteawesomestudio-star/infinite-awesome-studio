...
type ScenarioContent = { title: string; steps: string[]; note: string }

function buildEmployeeContent(DEMO_PROFILE: Client): Record<Scenario, ScenarioContent> {
  const co = DEMO_PROFILE.companyName
  return {
    eob: {
      title: 'Your claim was denied — here\'s what the EOB is actually saying',
      steps: [
        `An EOB (Explanation of Benefits) is not a bill. It's a summary of how ${DEMO_PROFILE.claims.medicalCarrier} processed your claim. "Denied" means the carrier didn't pay — not that you owe the provider yet.`,
        ...
        `Call ${DEMO_PROFILE.claims.medicalCarrier} Member Services at ${DEMO_PROFILE.claims.memberServicesPhone}. Ask them to explain the denial code in plain language and confirm whether an appeal is appropriate.`,
        `You have ${DEMO_PROFILE.claims.appealDeadlineDays} days from the denial date to file an internal appeal. Don't wait — get your provider's clinical notes and a letter of medical necessity if the denial was for "not medically necessary."`,
        `If the internal appeal is denied, you have the right to an external review by an independent organization. ${DEMO_PROFILE.claims.externalReviewAvailable ? `${co}'s plan participates in external review.` : 'Ask HR about external review options.'}`,
      ],
    ...
    'prior-auth': {
      ...
      steps: [
        `Prior authorization (PA) means ${DEMO_PROFILE.claims.medicalCarrier} must approve a service before it happens. Without it, the claim will be denied even if the service is medically necessary.`,
        `Ask your provider if PA is required before scheduling. ${DEMO_PROFILE.claims.priorAuthList}. Your provider's office typically submits the PA request — you don't have to do it yourself.`,
        ...
        `To appeal a PA denial: have your provider submit additional clinical notes, a letter of medical necessity, and any peer-reviewed literature supporting the treatment. Appeals go to ${DEMO_PROFILE.claims.medicalCarrier} — call ${DEMO_PROFILE.claims.memberServicesPhone} to start.`,
        `If the service was urgent (needed within 72 hours), you may qualify for an expedited appeal — ${DEMO_PROFILE.claims.medicalCarrier} must respond within 72 hours. Request it explicitly when you call.`,
      ],
    ...
    'balance-billing': {
      ...
      steps: [
        ...
        `${DEMO_PROFILE.claims.ideoContractStatus} If you were treated at an in-network facility and received a surprise bill from an out-of-network anesthesiologist, radiologist, or assistant surgeon, the NSA likely protects you.`,
        `Your cost-sharing should be limited to what you'd pay for an in-network provider. If you received a higher bill, call ${DEMO_PROFILE.claims.medicalCarrier} at ${DEMO_PROFILE.claims.memberServicesPhone} and say "I believe this is a balance billing violation under the No Surprises Act."`,
...
function buildHrContent(DEMO_PROFILE: Client): Record<Scenario, ScenarioContent> {
  const co = DEMO_PROFILE.companyName
  return {
    eob: {
      title: 'Employee comes to HR with an EOB denial — your workflow',
      steps: [
        `HR's role in a claim denial is to help the employee navigate the appeal process, not to adjudicate the claim. Direct them to ${DEMO_PROFILE.claims.medicalCarrier} Member Services (${DEMO_PROFILE.claims.memberServicesPhone}) as the first step.`,
        ...
        `Remind the employee of the appeal window: ${DEMO_PROFILE.claims.appealDeadlineDays} days from the denial date for an internal appeal. If the employee is close to the deadline, flag it as urgent and help them draft a written appeal promptly.`,
        `If the employee has already been through internal appeal and it was denied, inform them of their right to external review. ${DEMO_PROFILE.claims.externalReviewAvailable ? `${DEMO_PROFILE.claims.medicalCarrier}'s plan supports external review. The ERISA/ACA external review process is independent and binding.` : 'Contact the carrier for external review options.'}`,
    ...
    'prior-auth': {
      ...
      steps: [
        `Prior auth decisions are made by ${DEMO_PROFILE.claims.medicalCarrier}, not by ${co}. HR cannot override or expedite a PA determination — but you can help the employee understand the appeal path.`,
    ...
    'balance-billing': {
      ...
      steps: [
        ...
        `${DEMO_PROFILE.claims.nsaCompliant ? `${co}'s ${DEMO_PROFILE.claims.medicalCarrier} plan is NSA-compliant.` : 'Verify NSA compliance with your broker.'} If the situation is NSA-protected, the employee's cost-sharing should be based on the in-network rate only.`,
        `Direct the employee to call ${DEMO_PROFILE.claims.medicalCarrier} at ${DEMO_PROFILE.claims.memberServicesPhone} and specifically use the phrase "No Surprises Act balance billing." This routes them to the correct team.`,
...
function buildQuickLookups(DEMO_PROFILE: Client): { q: string; a: string }[] {
  return [
    { q: 'What\'s the appeal deadline?', a: `${DEMO_PROFILE.claims.appealDeadlineDays} days from denial date` },
    { q: 'Carrier member services', a: DEMO_PROFILE.claims.memberServicesPhone },
    { q: 'Member portal', a: DEMO_PROFILE.claims.memberPortal },
    { q: 'Prior auth list', a: `${DEMO_PROFILE.claims.medicalCarrier} provider portal` },
    { q: 'External review available?', a: DEMO_PROFILE.claims.externalReviewAvailable ? 'Yes' : 'No' },
  ]
}
...
export default function ClaimsCompassDemo() {
  ...
  const content = mode === 'employee' ? buildEmployeeContent(DEMO_PROFILE)[scenario] : buildHrContent(DEMO_PROFILE)[scenario]
  const quickLookups = buildQuickLookups(DEMO_PROFILE)

  const buildMarkdown = () => {
    const scenarioLabel = SCENARIOS.find(s => s.id === scenario)?.label ?? scenario
    return `# ${DEMO_PROFILE.companyName} — Claims Compass (${scenarioLabel} · ${mode === 'employee' ? 'Employee guidance' : 'HR admin workflow'})\n\n## ${content.title}\n${content.steps.map((s, i) => `${i + 1}. ${s}`).join('\n')}\n\n> Note: ${content.note}\n`
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-4 items-center">
        ...
        <ExportBar
          ...
          filename={`${DEMO_PROFILE.companyName.replace(/\s+/g, '-')}-claims-${scenario}-${mode}.md`}
          accent="#F97316"
          className="ml-auto"
        />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-dark-card border border-dark-border rounded-2xl p-4">
            <p className="text-xs font-body font-semibold uppercase tracking-widest text-dark-muted mb-3">{DEMO_PROFILE.companyName} claims quick ref</p>
            <div className="space-y-2.5">
              {quickLookups.map((item, i) => (
            ...
            <div className="border-t border-dark-border mt-3 pt-3">
              ...
              <p className="text-xs text-dark-text">{DEMO_PROFILE.claims.medicalCarrier}</p>
            </div>
          </div>
          ...
          {mode === 'employee' && (
            ...
            >
              ...
              <p className="text-xs font-body text-dark-muted leading-relaxed">
                Contact {DEMO_PROFILE.companyName} HR at{' '}
                <span className="text-dark-text">{DEMO_PROFILE.claims.hrClaimsContact}</span>
                {' '}or call {DEMO_PROFILE.claims.medicalCarrier} directly at{' '}
                <span className="text-dark-text">{DEMO_PROFILE.claims.memberServicesPhone}</span>.
              </p>
            </div>
          )}
        </div>
