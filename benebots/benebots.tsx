...
async function storageSet(key, value) {
  try { if (!window.storage) return false; await window.storage.set(key, value); return true; } catch { return false; }
}
    ...
    <nav style={{backgroundColor:"#1A1A1A", fontFamily:"Inter,system-ui,sans-serif"}} className="w-56 flex-shrink-0 text-white h-screen sticky top-0 flex flex-col">
      <div className="px-4 py-4 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          {/* BeneBots robot head mark, per brand */}
...
export default function BeneBots() {
  ...
  // Load Inter + Space Grotesk from Google Fonts once
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      const s = await storageGet("benebots:state:v2");
      if (s) { try { const p = JSON.parse(s); if(p.client) setClient(p.client); if(p.qaChat) setQaChat(p.qaChat); if(p.drafts) setDrafts(p.drafts); if(p.loaChat) setLoaChat(p.loaChat); if(p.loaContext) setLoaContext(p.loaContext); } catch {} }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => storageSet("benebots:state:v2", JSON.stringify({ client, qaChat, drafts, loaChat, loaContext })), 500);
